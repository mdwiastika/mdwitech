...
const Contact = () => {
  const onSubmit = (event) => {
    ...
    const link = `https://wa.me/62895339390753/?text=${encodeURIComponent("Halo, nama saya " + name + ". Saya ingin " + subject + ". \nDetailnya sebagai berikut:\n" + message)}`;
    ...
    <div className="h-full bg-primary/30">
      <div className="container mx-auto py-32 text-center xl:text-left flex items-center justify-center h-full">
        <div className="flex flex-col w-full max-w-[700px]">
          ...
          <motion.form onSubmit={onSubmit} variants={fadeIn("up", 0.4)} initial="hidden" animate="show" exit="hidden" action="" className="flex flex-1 flex-col gap-6 w-full mx-auto">
            <div className="flex gap-x-6 w-full">
              <input type="text" className="input" placeholder="name" name="name" id="name" />
              <input type="email" className="input lowercase" placeholder="email" name="email" id="email" />
            </div>
            <input type="text" className="input" placeholder="subject" name="subject" id="subject" />
            <textarea name="message" id="message" placeholder="message" className="textarea normal-case" cols="30" rows="10"></textarea>
